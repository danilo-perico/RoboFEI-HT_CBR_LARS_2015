...
*/
       			...
       			CvScalar minG = cvScalar(19, 111 , 49, 0);//24 160 //160
        		CvScalar maxG = cvScalar(32, 255, 255, 0);//47 //250

       			//CvScalar minG = cvScalar(25, 62 , 142, 0);//gol já calibrado
        		//CvScalar maxG = cvScalar(35, 182, 230, 0);//gol já calibrado
		//}
...
//*******************************************************************************
		IplImage*  GrayG  = cvCreateImage(tamanhoG, IPL_DEPTH_8U, 1);
		cvCvtColor(GoalFrame, GrayG, CV_BGR2GRAY);
		double brightnessG =cvMean(GrayG);
		if (brightnessG < 30)
		{
		VISION_STATE = 1;
		cout << "Esta Escuro!!!"<< endl; 
...
//	linhas = cvHoughlines2( Imagem de borda, memoria, método , Rho , Theta     , Threshould , parametro 1 = 0 , parametro 2 = 0)
   ...
   for( i = 0; i < MIN(lines->total,100); i++ )
	    ...
	    if(cont_GoalSearch > 5) //Bola não foi encontrada
	    {
	    	std::cout<<"Goal Não encontrado"<<std::endl;
		VISION_LOST_GOAL=1;
	    }
        }
        else
	    ...
	    VISION_MOTOR2_GOAL = dxl_read_word( 2, P_PRESENT_POSITION_L);
	    std::cout <<"Servo2 Gol = "<< VISION_MOTOR2_GOAL << std::endl;

	    VISION_MOTOR1_GOAL = dxl_read_word( 1, P_PRESENT_POSITION_L);
...
static unsigned int varredura=0;
        ...
        dxl_write_word(2, MOVING_SPEED, 100);//Seta as velocidades da cabeça
